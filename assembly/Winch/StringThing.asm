;***********************************************************************************************
;*
;* StringThing.asm - String Thing Main Source File
;*
...
;*	Variables
	...
	cblock	H'20'
		...
		CoilCode			; this is the current coil status set to motor
...
Setup
	...
	movlw	H'30'			; setup the four motor coil outputs
	Banksel	TRISC
	movwf	TRISC
	...
	BANK0
	movlw	H'00'
	movwf	CoilCode		; load the first coil code

Loop
;* In version 2.0 the main loop code has been replaced with a computed goto
;
;	movf	ExecuteCmd, w
;	andlw	H'FF'
;	btfsc	STATUS, Z
...
;	goto	SetAddress
;	btfsc	ExecuteCmd, 0	; if the 0 bit of the command is set inc code
;	goto	IncCode
;	decf	CoilCode, f
;	goto	WrapCode
;IncCode
...
MotorOn
	movf	CoilCode, w		; get the Coil Code
	call	GetPattern
	Banksel	PORTC
	movwf	PORTC			; and put it out to the pins
	goto	CmdDone
...
GetPattern
	BANK0
	movlw 	high Message	; set high order byte of program
	movwf 	PCLATH 			; counter appropriately
	movf 	CoilCode, W 	; recall the character
	addlw 	low Message		; add command variable to ROM address
	btfsc 	STATUS, C 		; beginning command Table
	incf 	PCLATH, F 		; overflows? yes, increment PCLATH
	movwf 	PCL 			; move computed goto value into PC
